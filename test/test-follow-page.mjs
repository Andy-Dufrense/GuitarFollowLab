...
// ── 1b. 跳过第 1 个音，只弹第 2 个音（D4）──────────────────────────────────
// 期望（2026-09-22 定的）：跳过去的那个音记**漏**（不算弹错），这一下**改判给第 2 个音**并判对。
// 这条同时管住两件事：快速邻音（E4/F4）对号错一位、以及"漏一格之后全体错位一位"。
{
  const r = await runPage('1b. 跳过第 1 个音，只弹第 2 个音 D4', [{ midi: 62, at: 4.7 }], 5600);
  ok(r.good === 1 && r.bad === 0,
    `第二音该判对、跳过的第一音记漏（不是错），实际 对${r.good}/错${r.bad}/测不准${r.unclear}`);
}
