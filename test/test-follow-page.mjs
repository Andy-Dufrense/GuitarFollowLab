...
// ── 1b/1c. "跳过去一个音"只允许出现在**快音段落**（2026-09-22 用户口径）─────────
// 快音段落（间隔 200ms）里漏弹一格 → 认成"跳过去"：跳过的记**漏**、这一下判给下一个音并判对；
// 慢的地方（600ms）把音弹错（弹了下一个音的音高）→ **必须判错**，不许系统替你解释成"你跳过了"。
{
  const fast = [
    { midi: 60, string: 2, fret: 1, t: 4.0, dur: 0.2, measure: 0 },
    { midi: 62, string: 2, fret: 3, t: 4.2, dur: 0.2, measure: 0 },
  ];
  const r = await runPage('1b. 快音段落（200ms）漏弹第 1 个音、只弹第 2 个音', [{ midi: 62, at: 4.2 }], 5600, { timeline: fast });
  ok(r.good === 1 && r.bad === 0,
    `快音段落该认成"跳过去"：第二音判对、第一音记漏，实际 对${r.good}/错${r.bad}/测不准${r.unclear}`);
}
{
  const r = await runPage('1c. 慢的地方把音弹成下一个音的音高（600ms）→ 必须判错', [{ midi: 62, at: 4.7 }], 5600);
  ok(r.bad === 1, `慢的地方弹错就该判错（不许当跳过去），实际 对${r.good}/错${r.bad}/测不准${r.unclear}`);
}
