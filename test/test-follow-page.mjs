...
{
  const r = await runPage('6. 检查模式：第二个音拖了 250ms', [
    { midi: 60, at: T0 / 1000 },
    { midi: 62, at: (T0 + 600 + 250) / 1000 },
  ], 5600, { mode: 'tempo' });
  // 口径（2026-09-22 用户明确）：跟节拍模式下，**规定的时间窗里没出现理想音就是错** ——
  // 拖 250ms 已经超出容许 ±150ms，所以第二个音要按错记（不是"音准过了只是拖拍"）。
  ok(r.good === 1 && r.bad === 1,
    `拖出时间窗该按错算（对1/错1），实际 对${r.good}/错${r.bad}/测不准${r.unclear}`);
